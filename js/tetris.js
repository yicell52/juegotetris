...
<li class="list-group-item"><strong>También puedes usar los botones si estás en móvil</strong></li>
    ...
    cleanGameBoardAndOverlapExistingPieces() {
        for (let y = 0; y < Game.ROWS; y++) {
            for (let x = 0; x < Game.COLUMNS; x++) {
                ...
                // Overlap existing piece if any / Superponga la pieza existente, si la hubiera
    ...
    /**
     *
     * @param point An object that has x and y properties; the coordinates shouldn't be global, but relative to the point / Un objeto que tiene propiedades xey; las coordenadas no deben ser globales, sino relativas al punto
     * @returns {boolean}
     */
    ...
    // It returns true even if the point is not valid (for example if it is out of limit, because it is not the function's responsibility) / Devuelve verdadero incluso si el punto no es válido (por ejemplo, si está fuera del límite, porque no es responsabilidad de la función)
    isEmptyPoint(x, y) {
    ...
    /**
     * Check if a point (in the game board) is valid to put another point there. / Compruebe si un punto (en el tablero de juego) es válido para poner otro punto allí.
     * @param point the point to check, with relative coordinates / el punto a comprobar, con coordenadas relativas
     * @param points an array of points that conforms a figure / una matriz de puntos que conforma una figura
     */
    ...
    figureCanRotate() {
        ...
        for (const rotatedPoint of newPointsAfterRotate) {
            if (
                !this.isValidPoint(rotatedPoint, this.currentFigure.getPoints())
            ) {
                return false;
            }
        }
...
class Utils {
    ...
    static getRandomColor() {
        return Game.COLORS[
            Utils.getRandomNumberInRange(0, Game.COLORS.length - 1)
        ];
    }
